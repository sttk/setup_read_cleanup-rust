...
mod tests_of_phased_error {
    ...
    fn test_new() {
        let e = PhasedError::new(
            Phase::Read,
            PhasedErrorKind::CannotCallUnlessPhaseSetup("method".to_string()),
        );
    ...
    fn test_debug() {
        ...
        assert_eq!(format!("{:?}", e), "setup_read_cleanup::PhasedError { phase: Read, kind: CannotCallUnlessPhaseSetup(\"method\") }");
        ...
        assert_eq!(format!("{:?}", e), "setup_read_cleanup::PhasedError { phase: Read, kind: CannotCallUnlessPhaseSetup(\"method\"), source: Custom { kind: Interrupted, error: \"oh no!\" } }");
    }
    ...
    fn test_display() {
        ...
        assert_eq!(
            format!("{}", e),
            "[Read] CannotCallUnlessPhaseSetup(\"method\")"
        );
