...
mod tests_of_phased_error {
    ...
    fn test_new() {
        let e = PhasedError::new(
            Phase::Setup,
            PhasedErrorKind::CannotCallUnlessSetupUnlock("method".to_string()),
        );
    ...
    fn test_debug() {
        ...
        assert_eq!(format!("{:?}", e), "setup_read_cleanup::PhasedError { phase: Setup, kind: CannotCallUnlessSetupUnlock(\"method\") }");
        ...
        assert_eq!(format!("{:?}", e), "setup_read_cleanup::PhasedError { phase: Setup, kind: CannotCallUnlessSetupUnlock(\"method\"), source: Custom { kind: Interrupted, error: \"oh no!\" } }");
    }
    ...
    fn test_display() {
        ...
        assert_eq!(
            format!("{}", e),
            "[Setup] CannotCallUnlessSetupUnlock(\"method\")"
        );
